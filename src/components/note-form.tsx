...
import type { Note, NotePriority, NoteInputData, AttachmentFile } from "@/types/note"; // Import NoteInputData
...
interface NoteFormProps {
    existingNote?: Note;
    onSuccess?: () => void; // Callback after successful API save/update
...
export function NoteForm({ existingNote, onSuccess }: NoteFormProps) {
    // Removed isSubmittingExternal
    const { toast } = useToast();
    const [attachments, setAttachments] = useState<AttachmentFile[]>([]); // Keep state for potential future use
    ...
    const form = useForm<NoteFormValues>({
        ...
        defaultValues: {
            ...
            priority: existingNote?.priority || "low",
            content: "",
            createdAt: new Date(),
        },
    ...
    }, [existingNote, form]); // Rerun when existingNote or form instance change

    // Store current selection range to restore after applying format
    ...
    const handleContentChange = (event: React.FormEvent<HTMLDivElement>) => {
        ...
        // Restore selection asynchronously after state update and potential re-render
        // Use requestAnimationFrame or setTimeout to ensure it runs after DOM updates
        requestAnimationFrame(() => {
            restoreSelection();
            // Ensure the editor retains focus after input
            editorRef.current?.focus();
        });
    };
    ...
    const downloadAttachment = async (file: AttachmentFile) => {
        console.log("Downloading attachment:", file);
        if (!file._id) {
            toast({
                title: "Download Error",
                description: "File does not have an ID.",
                variant: "destructive",
            });
            return;
        }

        const response = await fetch(`/api/attachments/${file._id}`, {
            method: "GET",
        });
        ...
        if (response.status !== 200) {
            ...
            toast({
                title: "Download Error",
                description: message ?? "Failed to download the attachment.",
                variant: "destructive",
            });
            return;
        }
    ...
    const onSubmit = async (data: NoteFormValues) => {
        ...
        const attachmentData = await BPromise.map(
            attachments,
            async (file: File) => {
            ...
            { concurrency: 5 } // Limit concurrency to avoid overloading server
        );
        const noteApiData: NoteInputData = {
            ...
            attachments: attachmentData, // Handle attachments if sending data
        };
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    errorData.message || `Failed to ${existingNote ? "update" : "create"} idea.`
                );
            }
            ...
            onSuccess?.(); // Call parent callback (e.g., close dialog, trigger list reload)
        } catch (error: any) {
            toast({
                title: "Error",
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormItem>
                    {(existingNote?.attachments.length === 0 || !existingNote) && (
                        <>
                        ...
                        <div className="mt-4 space-y-2">
                            ...
                            <ul className="list-none p-0 m-0 space-y-1">
                                {attachments.map((file, index) => (
                                    <li
                                        key={index}
                                        className="flex items-center cursor-pointer justify-between text-sm p-2 border rounded-md bg-secondary/50"
                ...
                <div className="flex justify-end">
                    <Button
                        type="submit"
                        disabled={isSaving || !form.formState.isValid || !form.getValues("content")}
                    >
