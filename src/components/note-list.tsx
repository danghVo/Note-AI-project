...
async function saveOrUpdateNoteAPI(noteData: Note) {
    ...
    const body = JSON.stringify({
        ...
        createdAt: noteData.createdAt, // Send ISO string
    ...
    if (!response.ok) {
        ...
        throw new Error(
            errorData.message ||
                `Failed to ${method === "PUT" ? "update" : "create"} note: ${response.statusText}`
        );
    }
...
export function NoteList({ onRegisterReload }: NoteListProps) {
    ...
    const loadNotes = useCallback(async () => {
        ...
        try {
            ...
            setError(
                "Oops! Something went wrong while loading your notes. Please try refreshing the page."
    ...
    const handleSwapNotes = async (note1Id: string, note2Id: string) => {
        ...
        if (note1Index === -1 || note2Index === -1) {
            toast({
                title: "Swap Error",
                description: "Could not find notes to swap.",
                variant: "destructive",
            });
            setIsSwapping(false);
            return;
        ...
        try {
            ...
            const updateNote1Data: Note = {
                ...note1,
                title: note2.title,
                content: note2.content,
                priority: note2.priority,
            };
            const updateNote2Data: Note = {
                ...note2,
                title: note1.title,
    ...
    if (error) {
        return (
            <Alert variant="destructive">
                ...
                {/* Display the user-friendly error state */}
            </Alert>
        );
    }
            ...
            <div className="text-center py-10">
                ...
                <p className="mt-4 text-muted-foreground">
                    {searchTerm
                        ...
                        : "You haven't sparked any notes yet. Create one!"}
                </p>
            </div>
        );
    }
    ...
    return (
        <div
            className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 transition-opacity duration-300 ${
                loading || isSwapping ? "opacity-50 pointer-events-none" : "opacity-100"
            }`}
        >
            {notes.map((note) => (
                <div
